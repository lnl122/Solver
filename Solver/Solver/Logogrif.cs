...
namespace Solver
{
    class Logogrif
    {
        ...
        public Program.words Logogrif_Process_One(string path)
        {
        ...
        public List<Program.words> Logogrif_Process(Program.Picture_data d)
        {
            ...
            foreach (string t2 in L2) { Tasks2.Add(Task<Program.words>.Factory.StartNew(() => Logogrif_Process_One(t2))); }
        ...
        public List<Program.words> Logogrifs_Process(Program.Pictures_data d) // вход структура с урлами всех картинок, без колонок/строк и прочего выход - список структур о словах
        {
            var Tasks2 = new List<Task<List<Program.words>>>();
            foreach (Program.Picture_data p1 in d.pics)
            {
                Tasks2.Add(Task<List<Program.words>>.Factory.StartNew(() => Logogrif_Process(p1)));
            }
            Task.WaitAll(Tasks2.ToArray());
            ...
            if (d.Auto.Checked)
            {
                ...
                for (int i1=0; i1 < wi; i1++)
                {
                    ...
                    for (int i2 = 0; i2 < wi; i2++)
                    {
                        ...
                        List<string> logo1 = is_logogrif(w[i1].w_find, w[i2].w_find);
                ...
                for (int i1 = 0; i1 < wi; i1++)
                {
                    w[i1].w_find.AddRange(w[i1].w_base);
                    w[i1].w_find.AddRange(w[i1].w_assoc);
                }
                while (Program.input_busy) { System.Threading.Thread.Sleep(1000); }
                Program.input_busy = true;
                for (int i1 = 0; i1 < wi; i1++)
            ...
            Program.Mainform.BeginInvoke(new Action(() => Logogrif_Buttons_Enable(d)));
            Program.Mainform.BeginInvoke(new Action(() => Logogrif_Show_Anwers(d, r)));

            return r;
        }

        private List<string> is_logogrif(List<string> w1, List<string> w2)
        {
            ...
            foreach (string s1 in w1)
            {
                ...
                {
                    if (s1.Length != (s2.Length+1)) { continue; }
        ...
        private void Logogrif_Show_Anwers(Program.Pictures_data d, List<Program.words> res)
        {
            ...
            Event_Logogrif_ChangeSize(null, null);
        }
        public Program.Pictures_data Data;
        public Logogrif(int level, List<string> urls)//для только решения картинок
        {
            ...
            Data.BtnClose.Click += new EventHandler(Event_Logogrif_Close_Click);
            ...
            Data.init_num.ValueChanged += new EventHandler(Event_Logogrif_InitNum_Change);
            ...
            Data.pics_list.SelectedIndexChanged += new EventHandler(Event_Logogrif_ListPics_Select);
            ...
            Data.cb_protect.SelectedIndexChanged += new EventHandler(Event_Logogrif_Protect_Change);
            ...
            Data.cb_str.SelectedIndexChanged += new EventHandler(Event_Logogrif_Str_Change);
            ...
            Data.cb_col.SelectedIndexChanged += new EventHandler(Event_Logogrif_Col_Change);
            ...
            Event_Logogrif_ChangeSize(null, null);
            Program.Mainform.SizeChanged += new EventHandler(Event_Logogrif_ChangeSize);
            Program.Tabs.Controls.Add(Data.Tab);
            Program.Tabs.SelectTab(Program.Tabs.TabCount - 1);
        }

        private void Event_Logogrif_Protect_Change(object sender, EventArgs e)
        {
        ...
        private void Event_Logogrif_ListPics_Select(object sender, EventArgs e)
        {
        ...
        private static void Logogrif_Buttons_Enable(Program.Pictures_data d)                  // меняем оптом доступность кнопок
        {
        ...
        private static void Logogrif_Buttons_Disable(Program.Pictures_data d)                  // меняем оптом доступность кнопок
        {
        ...
        private void Event_Logogrif_Close_Click(object sender, EventArgs e)
        {
            Data.Tab.Dispose();
        }
        ...
        private void Event_Logogrif_InitNum_Change(object sender, EventArgs e)
        {
            Data.pics[Data.pics_list.SelectedIndex].init_num = Convert.ToInt32(Data.init_num.Value);
        }
        private void Event_Logogrif_Str_Change(object sender, EventArgs e)
        {
            Data.pics[Data.pics_list.SelectedIndex].str = Data.cb_str.SelectedIndex;
        }
        private void Event_Logogrif_Col_Change(object sender, EventArgs e)
        {
            Data.pics[Data.pics_list.SelectedIndex].col = Data.cb_col.SelectedIndex;
        }
        private void Event_Logogrif_Solve_Click(object sender, EventArgs e)
        {
            ...
            Logogrif_Buttons_Disable(Data);
            Program.Log("Начали решать картинки\r\n.\r\n");
            Task<List<Program.words>> t1 = Task<List<Program.words>>.Factory.StartNew(() => Logogrifs_Process(Data));
        }
